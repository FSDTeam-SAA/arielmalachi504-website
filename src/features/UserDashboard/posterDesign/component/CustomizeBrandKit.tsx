...
export type ColorEntry = { color: string; hex: string };
...
export default function CustomizeBrandKit({
  ...
  const handleColorChange = (index: number, value: string) => {
    setColors((prev) =>
      prev.map((item, i) =>
        i === index
          ? { ...item, color: value, hex: value.toUpperCase() }
          : item,
      ),
    );
  };
  ...
  const handleAddColor = () => {
    if (colors.length < 5) {
      setColors((prev) => [...prev, { color: "#4C8CF0", hex: "#4C8CF0" }]);
    }
  };
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center overflow-y-auto bg-[#1f2a44]/60 p-4 backdrop-blur-sm md:p-6">
      <div className="relative max-h-[90vh] w-full max-w-[1200px] overflow-y-auto rounded-2xl border border-[#d8e4ef] bg-[#f7fbff] p-4 shadow-[0_20px_60px_rgba(44,87,140,0.15)] md:p-5">
        <button
          onClick={onClose}
                  ...
                  {colors.map((item, index) => (
                    <div key={index} className="flex items-center gap-3">
                      ...
                      <button
                        ...
                        className="flex h-[42px] w-[42px] cursor-pointer items-center justify-center rounded-lg border border-[#fee2e2] bg-[#fff5f5] text-[#f87171] transition hover:bg-[#fee2e2] hover:text-[#ef4444] disabled:cursor-not-allowed disabled:opacity-40"
                      >
